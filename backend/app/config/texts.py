...
class Texts:
    ...
    MYID_RESPONSE = (
        ...
        "Добавьте это число в переменную OPERATORS в настройках бота (Railway), "
        "чтобы нажимать кнопки «Взять в работу» и «Запросить детали» в группе поддержки."
    )
    OPERATOR_NEED_ID = (
        "Чтобы нажимать эти кнопки, добавьте свой Telegram ID в OPERATORS в настройках бота.\n\n"
        "Ваш ID: {user_id}\n"
        "В личке с ботом отправьте /myid — там тоже покажет ID."
    )
    AM_I_OPERATOR_YES = "Да, вы в списке операторов. Кнопки в группе поддержки должны работать."
    AM_I_OPERATOR_NO = (
        "Нет. Ваш Telegram ID: {user_id}\n\n"
        "Добавьте именно это число в переменную OPERATORS в Railway (Variables → OPERATORS), "
    ...
    HELP_TEXT = (
        ...
        "/myid — показать ваш Telegram ID (для настройки операторов)\n"
        "/am_i_operator — проверить, в списке ли вы как оператор\n"
        "/help — эта справка\n\n"
